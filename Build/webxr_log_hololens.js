(function () {
  ...
  XRManager.prototype.getXRGamepads = function(frame, inputSources, refSpace) {
    ...
    if (!inputSources || !inputSources.length) {
      console.log('no inputSources');
      return vrGamepads;
    }
    console.log('inputSources.length ' + inputSources.length);
    for (var i = 0; i < inputSources.length; i++) {
      let inputSource = inputSources[i];
      // Show the input source if it has a grip space
      if (inputSource.gripSpace) {
        console.log('inputSource.gripSpace');
      } else {
        console.log('no inputSource.gripSpace');
      }
      if (inputSource.gamepad) {
        console.log('inputSource.gamepad');
      } else {
        console.log('no inputSource.gamepad');
      }
      if (inputSource.targetRaySpace) {
        console.log('inputSource.targetRaySpace');
      } else {
        console.log('no inputSource.targetRaySpace');
      }
